...
const empty = (hint?: string): CardData => ({
   loading: false,
   value: "—",
   hint,
});
...
export default function WorkerDashboardHome() {
   ...
   const deptCard: CardData = useMemo(() => {
      ...
      return all.size === 0
         ...
         : {
              loading: false,
              value: String(all.size),
              hint: all.size === 1 ? "department" : "departments",
           };
   }, [status, user]);
      ...
      <div className="space-y-8">
         <div>
            ...
            <p className="text-gray-500">
               Here&apos;s your personal snapshot. Drill into a department for
               executive tools you have access to.
            </p>
         </div>
            ...
            <CardContent className="flex flex-wrap gap-3">
               <Link href="/dashboard/departments">
                  <Button variant="outline" className="gap-2">
                     <Building2 className="h-4 w-4" /> My Departments{" "}
                     <ArrowRight className="h-4 w-4" />
                  </Button>
               </Link>
               <Link href="/dashboard/attendance">
                  <Button variant="outline" className="gap-2">
                     <UserCheck className="h-4 w-4" /> My Attendance{" "}
                     <ArrowRight className="h-4 w-4" />
                  </Button>
               </Link>
         ...
         <Card>
            ...
            <CardContent className="text-sm text-gray-500 space-y-1">
               <p>
                  Announcements, tasks, notifications, and calendar are
                  scheduled for the next waves.
               </p>
            </CardContent>
         </Card>
      </div>
