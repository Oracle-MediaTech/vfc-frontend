...
export default function AttendancePage() {
   ...
   const [editAttendance, setEditAttendance] = useState<IAttendance | null>(
      null,
   );
   ...
   const handleMarkAttendance = async (userId: string, sessionId: string) => {
      const markedAt = pastMarkAt
         ? new Date(pastMarkAt).toISOString()
         : undefined;
      const serviceOrder =
                  ...
                  {searchResults.map((user) => (
                     <li key={user.id} className="">
                        <button
                           type="button"
                           onClick={() =>
                              handleMarkAttendance(user.id!, sessionId)
                           }
