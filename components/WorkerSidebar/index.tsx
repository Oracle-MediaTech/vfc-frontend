...
const WorkerSidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  // Setup mobile state and track window resizing
  useEffect(() => {
    setMounted(true);
    const handleResize = () => {
  ...
  return (
    <>
      ...
      {/* Mobile Sticky Top Hamburger Menu Button */}
      ...
      >
        <div className="flex flex-col h-full justify-between">
              ...
              {WORKER_ITEMS.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    // Auto collapses menu state on click for both desktop & mobile screens
                    onClick={() => setIsOpen(false)}
                    className={cn(
          ...
          {/* Logout Trigger (Sticky Footer Bottom) */}
          <div className="px-1">
            <Button
              variant="ghost"
              className={cn(
